    ...
    <mu-list toggle-nested>
      <mu-list-item v-for="(db,index) of databases"
        ...
        <mu-list-item 
          ...
          :open="colOpen === i"
          slot="nested"
          @click="setDBTB(db.name,table.name)">
          <mu-list-item-action>
          ...
          <mu-list-item-action>
            ...
            <mu-list-item v-for="col of table.cols" :key="col.Field" button :ripple="false"  slot="nested">
              <mu-list-item-action>
...
export default {
  ...
  methods: {
    ...
    setDBTB(database, table) {
      this.$store.state.database = database;
      this.$store.state.table = table;
